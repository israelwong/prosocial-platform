...
async function main() {
    ...
    // Seed de canales de adquisición
    await seedCanalesAdquisicion();

    // Seed de plataformas de publicidad
...
async function seedRedesSociales() {
    ...
    const plataformas = [
        ...
        },
        {
            ...
            name: 'LinkedIn',
            slug: 'linkedin',
            description: 'Red social profesional y de networking',
            color: '#0A66C2',
            icon: 'linkedin',
...
async function seedDemoStudio() {
    ...
    const demoProject = await prisma.projects.upsert({
        ...
        create: {
            ...
            planId: 'basic', // Referencia al plan básico por slug
            active: true,
            updatedAt: new Date()
        }
    });
    ...
    for (const user of demoUsers) {
        ...
        // Crear perfiles profesionales para los usuarios empleados
        if (user.type === 'EMPLEADO') {
            ...
            await prisma.project_user_professional_profiles.upsert({
    ...
    for (const socialNetwork of demoSocialNetworks) {
        await prisma.project_redes_sociales.upsert({
