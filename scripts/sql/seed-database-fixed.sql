-- 🌱 Seed de Base de Datos con Nueva Arquitectura Supabase Auth (CORREGIDO)
-- Ejecutar con: source .env.local && psql $DATABASE_URL -f scripts/sql/seed-database-fixed.sql
...
-- 5. Insertar perfiles profesionales (usando profileId en lugar de profile)
INSERT INTO project_user_professional_profiles (id, "userId", "profileId", description, "isActive", "createdAt", "updatedAt")
VALUES 
    ('profile-photographer', 'demo-photographer', 'fotografo-profile', 'Perfil profesional de Juan Pérez', true, NOW(), NOW()),
    ('profile-editor', 'demo-editor', 'editor-profile', 'Perfil profesional de María García', true, NOW(), NOW())
ON CONFLICT (id) DO UPDATE SET
...
-- 6. Insertar leads de ejemplo (sin monthlyBudget)
INSERT INTO platform_leads (id, name, email, phone, "studioName", "studioSlug", "lastContactDate", "interestedPlan", "probableStartDate", priority, "stageId", "acquisitionChannelId", "firstInteractionDate", "originalSource", "interactionCount", "leadType", "createdAt", "updatedAt")
VALUES 
    ('lead-demo-1', 'Ana Martínez', 'ana.martinez@email.com', '+1234567894', 'Boda Ana & Carlos', NULL, NOW(), 'pro', NOW() + INTERVAL '30 days', 'high', 'stage-comercial-interesados', 'canal-referidos', NOW(), 'Referido por cliente anterior', 3, 'prospect', NOW(), NOW()),
    ('lead-demo-2', 'Roberto Silva', 'roberto.silva@email.com', '+1234567895', 'Sesión Familiar Silva', NULL, NOW(), 'basic', NOW() + INTERVAL '15 days', 'medium', 'stage-comercial-contactados', 'canal-redes-sociales', NOW(), 'Instagram', 1, 'prospect', NOW(), NOW())
ON CONFLICT (id) DO UPDATE SET
    name = EXCLUDED.name,
    email = EXCLUDED.email,
    phone = EXCLUDED.phone,
